...
import type { Variants } from "framer-motion"; // ✔ correct
...
// --- Optimized Variants ---
// Note: We define these outside the component to prevent re-creation on render
  ...
  return (
    <footer className="relative bg-[#030014] text-white overflow-hidden pt-20 pb-10 border-t border-white/5 font-sans">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {/* We animate this separately so it appears before the grid staggers */}
        <motion.div
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-light text-neutral-200 max-w-2xl leading-relaxed">
            ...
            <span className="text-indigo-400 font-medium">digital realities</span>{" "}
            for your business.
          </h2>
        </motion.div>

        {/* --- Main Grid Container --- */}
        {/* IMPORTANT: We set initial and whileInView HERE. 
            The children will inherit "hidden" and "visible" automatically. */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.2 }} // amount ensures it triggers when user sees 20% of it
        ...
        >
            ...
            <ul className="space-y-4">
              {servicesLinks.map((link) => (
                <li key={link}>
                  <Link
                    href="#"
                    className="group flex items-center gap-2 text-neutral-400 hover:text-indigo-300 transition-colors"
                  >
          ...
          <motion.div variants={itemVariants} className="lg:col-span-3 lg:pl-4">
            <div className="p-6 rounded-2xl bg-gradient-to-b from-white/10 to-transparent border border-white/10 text-center lg:text-left">
              ...
              {/* Note: This is a nested motion component. It's safe because it handles its own hover state */}
              <motion.button
          ...
          <div className="flex gap-6">
            {[Twitter, Linkedin, Instagram, Facebook].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="text-neutral-400 hover:text-indigo-400 transition-colors hover:scale-110 transform duration-200"
              >
